...
update model@Model {..} (Animate dt) =
  if playerStatus `notElem` [Playing, Dead]
    then (model, Cmd.none)
    else ( model
           { flapperPos =
               if y < -hh
...
update model@Model {..} Flap
  ...
  | playerStatus == Playing
    -- flap, if allowed
   =
    if stillFlapping timeScore lastFlap
      then (model, Cmd.none)
      else ( model {flapperVel = V2 0 (-flapAccel), lastFlap = Just timeScore}
           , Cmd.none)
  | otherwise = (model, Cmd.none)
...
deadOverlay color Model {..} =
  group
    [ move (V2 0 (-25)) $
      text $
      Text.height 30 $ Text.color color $ Text.toText "Oops, you are dead."
    , move (V2 0 25) $
...
playingOverlay color Model {..} =
  ...
  (if playerStatus == Paused
     then [centerNotice "Press P to resume"]
     else if timeScore < showPauseHelpFor
            then [centerNotice "Press P to pause"]
            else [])
  where
    status = secondsText timeScore ++ " | " ++ printf "%.2fx speed" timeSpeed
    centerNotice msg =
      move (V2 0 0) $ text $ Text.height 16 $ Text.color color $ Text.toText msg
    V2 _ h = fromIntegral <$> windowDims
...
view assets model@Model {..} =
  ...
  where
    ...
    --flapper = filled (rgb 0.36 0.25 0.22) $ rect flapperDims   -- mjh
    flapperSprite
      | playerStatus == Dead = "birdDead"
      | stillFlapping timeScore lastFlap = "birdFlap"
      | otherwise = "bird"
    flapper = image flapperDims (assets M.! flapperSprite) -- try this alternative 
      where
        V2 _ dy = flapperVel
...
-- http://hackage.haskell.org/package/helm-1.0.0/docs/Helm-Engine.html
